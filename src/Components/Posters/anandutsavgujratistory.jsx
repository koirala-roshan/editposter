...
const Anandutsavgujratistory = () => {
  ...
  const [Teachersname, setTeachersName] = useState('રોહિત શર્મા');
  const [link, setLink] = useState('નોંધણી કરો: aolic.org/12345');
  ...
  const [venue, setVenue] = useState('સ્થળ: આર્ટ ઓફ લિવિંગ ઇન્ટરનેશનલ સેન્ટર');
...
//downloadpostercode
  ...
  const downloadPoster = async () => {
    ...
    const poster = document.getElementById('posteranandgujratistory');
...
{/* <nav class="navbar fixedtop">
    ...
    <div className='container'>
      ...
      <div id="posteranandgujratistory" className='firstdiv' style={{ width: '810px', height: '1440px', border: '1px solid #000', padding: '20px' }}>

        <h5 id='teachersnameanandgujratistory'>{Teachersname}</h5>
        <h5 id='linkanandgujratistory'>{link}</h5>
        <h5 id='dateanandgujratistory'>{date}</h5>
        <h5 id='timeanandgujratistory'>{time}</h5>
        <h5 id='contactanandgujratistory'>{contact}</h5>
        <h5 id='venueanandgujratistory'>{venue}</h5>
...
export default Anandutsavgujratistory;